...
class Decoder(Module):
    ...
    def __init__(self, dims, dim, class_num=2):
        ...
        self.linear_fuse34 = ConvModule(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=1,
                                        norm_cfg=dict(type='BN', requires_grad=True))
        self.linear_fuse2 = ConvModule(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=1,
                                       norm_cfg=dict(type='BN', requires_grad=True))
        self.linear_fuse1 = ConvModule(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=1,
                                       norm_cfg=dict(type='BN', requires_grad=True))
    ...
    def forward(self, inputs):
        ...
        L34 = self.linear_fuse34(torch.add(_c4, _c3))
        L2 = self.linear_fuse2(torch.add(L34, _c2))
        _c = self.linear_fuse1(torch.add(L2, _c1))
...
class mit_srm_add(nn.Module):
    def __init__(self, class_num=2, **kwargs):
        super(mit_srm_add, self).__init__()
    ...
    def forward(self, x):
        ...
        features, x = self.decode_head(features)
...
MitEncoder = mit_srm_add(class_num=2)
